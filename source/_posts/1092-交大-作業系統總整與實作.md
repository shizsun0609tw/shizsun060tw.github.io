...
在 linux 系統中，Interrupt Handler 的處理會透過 Softirq 的方式執行
Interrupt Handler 被分為 Top Half 與 Bottom Half，Softirq 會把 Bottom Half 的部分在不同的 CPU 上去執行
在處理上會較快速且有效率，但也有可能造成兩個 CPU 互相 lock (一支程式會被兩個 CPU 執行)
常一起比較的為 Tasklet 最大不同是 Top 與 Bottom 會在同一個 CPU 上執行，因此不會產生 lock 的問題，但效率也較差
